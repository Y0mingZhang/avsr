...
class AdditiveAVConformerRNNTModule(AVConformerRNNTModule):
    """ AVConformerRNNT, but simple fusion by adding audio and visual encodings """
    def __init__(self, args=None, sp_model=None):
        super(AVConformerRNNTModule, self).__init__()
        self.save_hyperparameters(args)
        self.args = args
        self.sp_model = sp_model
        spm_vocab_size = self.sp_model.get_piece_size()
        ...
        self.optimizer = torch.optim.AdamW(
            itertools.chain(*([self.model.parameters()] + frontend_params)),
            lr=8e-4,
            weight_decay=0.06,
            betas=(0.9, 0.98),
        )
...
class VotingAVConformerRNNTModule(AVConformerRNNTModule):
    """ AVConformerRNNT, weighted voting version """
    def __init__(self, args=None, sp_model=None):
        super(AVConformerRNNTModule, self).__init__()
        self.save_hyperparameters(args)
        self.args = args
        ...
        frontend_params = [self.video_frontend.parameters(), self.audio_frontend.parameters(), [self.weight]]
    ...
    def _step(self, batch, _, step_type):
        ...
        video_weight = torch.sigmoid(self.weight)
        audio_weight = 1 - video_weight

        output, src_lengths, _, _ = self.model(
            video_features * video_weight + audio_features * audio_weight,
            batch.video_lengths,
            prepended_targets,
            prepended_target_lengths,
        )
